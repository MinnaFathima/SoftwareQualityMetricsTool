...
def analyze_uploaded_file(file_path):
    with open(file_path, 'r') as f:
        source_code = f.read()
    # Capture function name and complexity
    complexities = {func.name: func.complexity for func in cc_visit(source_code)}
    return complexities

# Store Complexity Data in JSON
def store_complexity_data(file_path, complexities, storage_file='complexity_data.json'):
    ...
    data[file_name][timestamp] = complexities
...
def predict_trend(data):
    ...
    for func_name, complexities in data.items():
        if len(complexities) < 2:
            predictions[func_name] = complexities[-1] if complexities else None
            continue
...
def process_uploaded_file(file_path, storage_file='complexity_data.json'):
    ...
    # Extract trend data per function in the file
    file_name = os.path.basename(file_path)
    if file_name not in data:
        raise ValueError(f"No data found for file: {file_name}")

    trend_data = {}
    for timestamp, func_complexities in data[file_name].items():
